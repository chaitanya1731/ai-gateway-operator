...
- At build time, `Dockerfile` copies these manifests into the container image at `/opt/manifests/`; an init container copies them into a writable emptyDir at runtime (see `config/manager/manager.yaml`).
